...
import com.predict.tide.black.tide.tideCode.module.circleFriend.adapter.bean.UserInfo;

import java.util.List;


/**
 * Created by 86084423 on 2018/4/24.
 */
...
public class CommentsView extends LinearLayout {
    ...
    private View getView(final int position) {
        ...
        addTextView(ll,item,position);
        return ll;
    }

    private void addTextView(LinearLayout ll,Comment item,int position){
        UserInfo replyUser = item.getReplyUser();
        ...
        UserInfo comUser = item.getCommentUser();
        String name = comUser.getUsername();
        if (hasReply) {
            builder.append(setClickableSpan(name, item.getCommentUser()));
            builder.append(" 回复 ");
...
//        textView.setHighlightColor(0xff000000);
            ...
            addTextView(ll,item.getComment(),position);
            ll.addView(textView);
        } else {
            builder.append(setClickableSpan(name, item.getCommentUser()));
            builder.append(" : ");
...
//        });
    ...
    public interface onItemClickListener {
        void onItemClick(int position, Comment bean);
    }
